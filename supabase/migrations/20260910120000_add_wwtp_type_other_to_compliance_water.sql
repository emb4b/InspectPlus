...
-- ─────────────────────────────────────────────────────────────────────────
-- compliance_water gains wwtp_type_other: what "Others" means, when that is
-- the answer to section 5B's "Type of WWTP" (Physical / Biological /
-- Chemical / Combined / Others). wwtp_type is a plain text column holding
-- only the chosen option, so picking "Others" recorded that the plant is
-- none of the listed kinds but not which kind it actually is — this column
-- gives that specify text somewhere to go.
--
...
-- simply omits it from its push payload. Existing rows keep NULL —
-- correct, since no inspection before this was asked the question.
--
-- Full body re-issued, based on
-- 20260909120000_add_non_wwtp_treatment_to_compliance_water.sql (the last
