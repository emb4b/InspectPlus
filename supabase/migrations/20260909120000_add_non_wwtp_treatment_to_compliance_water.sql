...
-- ─────────────────────────────────────────────────────────────────────────
-- compliance_water gains non_wwtp_treatment: what an establishment without
-- a WWTP actually treats its wastewater with. Section 5A of the water
-- report asks "Has WWTP?" first; on the "no" branch it now also asks which
-- system is in place — Septic Tank, Oil and Water Separator (OWS), or free
-- text under Others — since "no WWTP" was never the same claim as "no
-- treatment", and the report had nowhere to record the difference.
--
-- Shape: { systems: string[], other: string }, jsonb. A report that records
-- a WWTP stores {} here; the client narrows this at save time (see
-- nonWwtpTreatmentFor in src/features/inspections/water/waterTypes.ts) so
-- no row ever describes both a WWTP and a septic tank.
--
-- Additive per docs/sync-contract.md's Backward Compatibility Rules: the
-- column is nullable, and a client that has not yet shipped the field
-- simply omits it from its push payload, where the coalesce below defaults
-- it to {}. Existing rows keep NULL, which the app's asObject decoder reads
-- as {} — correct, since no inspection before this was asked the question.
--
-- Full body re-issued, based on 20260816080000_track_push_conflicts.sql
-- (the last migration to redefine push_changes) — the only changes are the
-- compliance_water insert column lists, their values, and the update loop's
-- on-conflict set.
-- ─────────────────────────────────────────────────────────────────────────

alter table public.compliance_water
  add column if not exists non_wwtp_treatment jsonb;
  ...
  loop
    begin
      insert into public.compliance_water (
        ...
        abstracted_water_quality, has_wwtp, non_wwtp_treatment, wwtp_type,
        wwtp_details, wwtp_components, wwtp_condition,
