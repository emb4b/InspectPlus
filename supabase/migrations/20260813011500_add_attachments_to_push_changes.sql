...
-- Add attachments to push_changes. Full function body re-issued (based on
-- 20260811120000_add_operating_status_since_to_establishments.sql, the last
-- migration to redefine push_changes) with new created/updated/deleted
-- loops for attachments appended, using the same last-write-wins
-- `updated_at` guard and soft-delete pattern already used for
...
begin
  ...
  loop
    insert into public.attachments (
      ...
      captured_at, device_id, created_at, updated_at, sync_status
    )
    values (
