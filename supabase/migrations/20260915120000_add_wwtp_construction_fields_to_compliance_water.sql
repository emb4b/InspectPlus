...
-- ─────────────────────────────────────────────────────────────────────────
-- compliance_water gains section 5E's remaining questions. The printed form
-- asks six things under "E. Condition of the WWDP"; the app stored only the
-- first two (wwtp_condition, wwtp_under_construction).
--
--   wwtp_condition_other              what "Others" means, when that is the
--                                     condition (question 1)
--   wwtp_construction_reported        has the construction/rehabilitation
--                                     been reported to EMB/LLDA? (3)
--   wwtp_construction_units           which system or units are under
--                                     construction or being modified (4)
--   wwtp_construction_completion_date estimated date of completion (5)
--   wwtp_treatment_units_utilized     treatment units utilized to treat
--                                     wastewater meanwhile (6)
--
-- Questions 3-6 only apply when question 2 is Yes; the client sends NULL
-- for all four otherwise, so the record never describes construction work
-- on a plant it also says is not under construction.
--
-- Additive per docs/sync-contract.md's Backward Compatibility Rules: every
-- column is nullable, and a client that has not yet shipped the fields
-- simply omits them from its push payload. Existing rows keep NULL —
-- correct, since no inspection before this was asked.
--
-- Full body re-issued, based on
-- 20260910120000_add_wwtp_type_other_to_compliance_water.sql (the last
-- migration to redefine push_changes) — because Postgres has no way to
-- alter one column of a function body, the whole thing must be redefined;
-- the only changes are the compliance_water insert column lists, their
-- values, and the update loop's on-conflict set.
-- ─────────────────────────────────────────────────────────────────────────

alter table public.compliance_water
  add column if not exists wwtp_condition_other text,
  add column if not exists wwtp_construction_reported boolean,
  add column if not exists wwtp_construction_units text,
  add column if not exists wwtp_construction_completion_date date,
  add column if not exists wwtp_treatment_units_utilized text;
  ...
  loop
    begin
      insert into public.compliance_water (
        ...
        abstracted_water_quality, has_wwtp, non_wwtp_treatment, wwtp_type, wwtp_type_other,
        wwtp_details, wwtp_components, wwtp_condition, wwtp_condition_other,
        wwtp_under_construction, wwtp_construction_reported,
        wwtp_construction_units, wwtp_construction_completion_date,
        wwtp_treatment_units_utilized,
      ...
      values (
        ...
        coalesce(compliance_water_record->'non_wwtp_treatment', '{}'::jsonb),
        ...
        compliance_water_record->>'wwtp_condition_other',
        coalesce((compliance_water_record->>'wwtp_under_construction')::boolean, false),
        (compliance_water_record->>'wwtp_construction_reported')::boolean,
        compliance_water_record->>'wwtp_construction_units',
        nullif(compliance_water_record->>'wwtp_construction_completion_date', '')::date,
        compliance_water_record->>'wwtp_treatment_units_utilized',
  ...
  loop
    begin
      ...
      on conflict (compliance_id) do update set
        ...
        non_wwtp_treatment = excluded.non_wwtp_treatment,
        ...
        wwtp_condition_other = excluded.wwtp_condition_other,
        wwtp_under_construction = excluded.wwtp_under_construction,
        wwtp_construction_reported = excluded.wwtp_construction_reported,
        wwtp_construction_units = excluded.wwtp_construction_units,
        wwtp_construction_completion_date = excluded.wwtp_construction_completion_date,
        wwtp_treatment_units_utilized = excluded.wwtp_treatment_units_utilized,
