...
-- push_changes has no way to tell you which specific row failed when an
-- insert hits an FK or RLS violation — Postgres's own error text for RLS
-- violations deliberately omits row values (see compliance_water's "new
-- row violates row-level security policy" investigation), and FK
-- violation DETAIL is also stripped by default in this environment. That
-- makes exactly the failure class this app has repeatedly hit (a child
-- row pushed whose parent didn't actually land server-side) nearly
-- impossible to diagnose from the client side alone.
--
-- Wrapping each `created`-row insert in its own BEGIN/EXCEPTION re-raises
-- the same underlying error but with the specific primary/foreign key
-- values embedded directly in the message (not just DETAIL, which gets
-- stripped) — so the next time this happens, the error the client
-- receives says exactly which row and which id failed.
--
-- Scoped to `created` loops only (not `updated`/`deleted`): those are the
-- ones that hit fresh INSERT ... WITH CHECK RLS evaluation and FK checks
-- against rows that may not exist yet; updates/deletes act on rows
-- already resolved by their WHERE clause and don't have the same "which
-- row" ambiguity.
--
-- Full body re-issued, based on 20260815010000_add_caption_to_attachments.sql
-- (the last migration to redefine push_changes) — no column/behavior
-- changes, only the exception wrapping described above.
...
begin
  ...
  loop
    ...
    set
      inspection_report_id = attachment_record->>'inspection_report_id',
      survey_report_id = attachment_record->>'survey_report_id',
      inspector_uid = attachment_record->>'inspector_uid',
      storage_path = attachment_record->>'storage_path',
      file_name = attachment_record->>'file_name',
      mime_type = attachment_record->>'mime_type',
      file_size = nullif(attachment_record->>'file_size', '')::bigint,
      geo_lat = nullif(attachment_record->>'geo_lat', '')::double precision,
      geo_lng = nullif(attachment_record->>'geo_lng', '')::double precision,
      captured_at = (attachment_record->>'captured_at')::timestamptz,
      caption = attachment_record->>'caption',
      device_id = attachment_record->>'device_id',
      updated_at = coalesce((attachment_record->>'updated_at')::timestamptz, now()),
      sync_status = coalesce((attachment_record->>'sync_status')::public.sync_status_enum, 'pending')
    where attachment_id = attachment_record->>'attachment_id'
      and coalesce((attachment_record->>'updated_at')::timestamptz, now()) >= updated_at;
  end loop;
