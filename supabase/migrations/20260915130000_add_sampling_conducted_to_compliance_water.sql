...
-- ─────────────────────────────────────────────────────────────────────────
-- compliance_water gains section 6I's gate. The form's "I. Water Quality
-- Sampling" went straight to listing sampling points; it now first asks
-- whether the inspector sampled at all, and if so what kind.
--
--   sampling_conducted       did the inspector conduct water quality
--                            sampling on this inspection?
--   sampling_classification  'Ambient', 'Effluent' or 'Both' when so
--
-- The client sends sampling_points as [] and the classification as NULL
-- whenever sampling_conducted is false, so the record never lists points
-- for sampling it also says did not happen.
--
-- Additive per docs/sync-contract.md's Backward Compatibility Rules: both
-- columns are nullable, and a client that has not yet shipped the fields
-- simply omits them from its push payload. Existing rows keep NULL and
-- their sampling points stand as they are — no inspection before this was
-- asked.
--
-- Full body re-issued, based on
-- 20260915120000_add_wwtp_construction_fields_to_compliance_water.sql (the
-- last migration to redefine push_changes) — because Postgres has no way to
...
alter table public.compliance_water
  add column if not exists sampling_conducted boolean,
  add column if not exists sampling_classification text;
  ...
  loop
    begin
      insert into public.compliance_water (
        ...
        sampling_conducted, sampling_classification,
      ...
      values (
        ...
        (compliance_water_record->>'sampling_conducted')::boolean,
        compliance_water_record->>'sampling_classification',
  ...
  loop
    begin
      ...
      on conflict (compliance_id) do update set
        ...
        sampling_conducted = excluded.sampling_conducted,
        sampling_classification = excluded.sampling_classification,
